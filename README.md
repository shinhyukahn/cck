...
일반 사용에서는 필요 없지만, 직접 호출할 수도 있습니다.
...
`/compact`, `/diff`, `/review`, `/rewind`, `/cost` 등 Claude Code 세션 안에서 직접 실행할 수 있는 커맨드는 설명 없이 바로 실행합니다.
...
`disable-model-invocation: true`로 설정되어 모델 호출 없이 즉시 치트시트를 출력합니다.
...
인자 없이 `/cck:setup`만 입력하면 유형을 물어봅니다. 자동 감지 불가능한 `[TODO]` 항목은 사용자가 직접 채울 수 있게 표시해서 남겨둡니다.

---

## 지원 워크플로우

`data/recipes.json`에 정의된 워크플로우입니다. "PR 올리고 싶어"처럼 말하면 `korean-autopilot`이 해당 레시피를 참조해서 단계별로 안내합니다.
...
cck/
├── .claude-plugin/
│   └── plugin.json              # 플러그인 매니페스트 (이름, 버전, 저자)
├── data/
...
자연어 키워드 → 커맨드 ID 배열. `korean-autopilot`이 사용자 입력에서 이 키워드를 감지하면 해당 커맨드를 우선 제안합니다.
...
| CLAUDE.md 템플릿 | `skills/setup/templates/`에 `.md` 파일 추가 |
| 버그 수정 | 이슈 등록 후 PR |
...
Commands with `session_executable: true` are executed directly in the session. Commands with `session_executable: false` are presented as copyable terminal instructions.

## Explicit Commands

```
/cck:cheat          Korean cheatsheet, rendered instantly (no model invocation)
/cck:setup          Generate a Korean CLAUDE.md for the current project
/cck:setup web-app  Generate a specific template directly
```
